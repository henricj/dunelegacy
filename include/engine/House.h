...
public:
    ...
    [[nodiscard]] bool isAlive() const noexcept {
        return (teamID == 0) || !(((numStructures - numItem[Structure_Wall]) <= 0) &&
                                  (((numUnits - numItem[Unit_Carryall] - numItem[Unit_Harvester] -
                                     numItem[Unit_Frigate] - numItem[Unit_Sandworm]) <= 0)));
    }
    ...
    */
    [[nodiscard]] bool isGroundUnitLimitReached() const {
        int numGroundUnit = numUnits - numItem[Unit_Soldier] - numItem[Unit_Trooper] - numItem[Unit_Carryall] -
    ...
    */
    ...
    static HOUSETYPE   getHouseByName(std::string_view name);
    static std::string getHouseNameByNumber(HOUSETYPE house);
