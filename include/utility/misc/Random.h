...
public:
    ...
    */
    uint32_t rand() {
        const int32_t IA = 16807;
        const int32_t IM = 2147483647;
        const int32_t IQ = 127773;
        const int32_t IR = 2836;
        const uint32_t MASK = 123459876;

        uint32_t k;

        int32_t idum = seed ^ MASK;
        k = idum / IQ;
    ...
    */
    ...
    T getRandOf(std::initializer_list<T> initlist) {
        return initlist.begin()[rand(0, static_cast<int32_t>(initlist.size()-1))];
    }
