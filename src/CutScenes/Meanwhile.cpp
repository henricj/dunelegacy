...
Meanwhile::Meanwhile(HOUSETYPE house, bool firstMeanwhile) {
    ...
    if (firstMeanwhile) {
        ...
        addTextEvent(
            std::make_unique<TextEvent>(dune_text.getString(textBaseIndex + MeanwhileText_At_the_Emperor_s_Palace),
        ...
        addTextEvent(
            std::make_unique<TextEvent>(dune_text.getString(textBaseIndex + MeanwhileText_Yes_your_excellency_I),
                                        visitorColor, 45, 30, true, false, false));

        startNewScene();

        addVideoEvent(std::make_unique<WSAVideoEvent>(pImperator.get()));
        addVideoEvent(
            std::make_unique<HoldPictureVideoEvent>(pImperator->getPicture(pImperator->getNumFrames() - 1).get(), 3));
        addTextEvent(std::make_unique<TextEvent>(dune_text.getString(textBaseIndex + MeanwhileText_You_let_the),
        ...
        addTextEvent(std::make_unique<TextEvent>(dune_text.getString(textBaseIndex + MeanwhileText_I_will_not_allow),
                                                 sardaukarColor, 37, 38, false, true, false));
        ...
        addTextEvent(std::make_unique<TextEvent>(
            dune_text.getString(textBaseIndex + MeanwhileText_At_the_Emperor_s_Palace_on_Dune), color, 0, 42, true,
            true, true));
        addTrigger(std::make_unique<CutSceneMusicTrigger>(0, MUSIC_MEANWHILE));
        ...
        if (house == HOUSETYPE::HOUSE_ATREIDES) {
            ...
            addTextEvent(
                std::make_unique<TextEvent>(dune_text.getString(textBaseIndex + MeanwhileText_And_still_you_fail),
                                            sardaukarColor, 50, 45, false, false, false));
            addTextEvent(std::make_unique<TextEvent>(dune_text.getString(textBaseIndex + MeanwhileText_But_excell),
                                                     visitorColor, 100, 30, true, false, false));
            ...
            addTextEvent(
                std::make_unique<TextEvent>(dune_text.getString(textBaseIndex + MeanwhileText_Enough_Together_we_must),
            ...
            addTextEvent(std::make_unique<TextEvent>(
                ...
                true, true, false));
            addTextEvent(std::make_unique<TextEvent>(dune_text.getString(textBaseIndex + MeanwhileText_Your_highness),
                                                     visitorColor, 46, 35, true, false, false));
