...
static const uint64_t scales[16] = {
    ...
    1000000ULL,     10000000ULL,    100000000ULL,   1000000000ULL, 10000000000ULL, 10000000000ULL,
    10000000000ULL, 10000000000ULL, 10000000000ULL, 10000000000ULL};
...
void fix32_to_str(fix32_t value, char* buf, int decimals) {
    const uint64_t uvalue = (value >= 0) ? value : -value;
    if (value < 0)
        *buf++ = '-';
