...
fix32_t fix32_sin_parabola(fix32_t inAngle) {
    fix32_t abs_inAngle = 0;

    fix32_t abs_retval = 0;
...
fix32_t fix32_atan2(fix32_t inY, fix32_t inX) {
    ...
    if (inX >= 0) {
        ...
        r_3   = fix32_mul(fix32_mul(r, r), r);
        angle = fix32_mul(0x0000000030840015LL, r_3) - fix32_mul(0x00000000F876CCE0LL, r) + fix32_THREE_PI_DIV_4;
    }
    if (inY < 0) {
        angle = -angle;
    }
