...
namespace
{
constexpr double opl_frequency = 14318180.0 / 288.0;
}
...
#if FLT_RADIX == 2 
	    const auto dbOriginalFreq = scalbln(opl_frequency * iFNum, iBlock - 20);
#else
        const auto dbOriginalFreq = opl_frequency * static_cast<double>(iFNum) * pow(2.0, iBlock - 20);
#endif
...
#if FLT_RADIX == 2
        const auto calcFNum = [&] {
            return (dbOriginalFreq + (dbOriginalFreq / this->offset)) / scalbn(opl_frequency, iNewBlock - 20);
        };
#else
        const auto calcFNum       = [&] {
            return (dbOriginalFreq + (dbOriginalFreq / this->offset)) / (opl_frequency * pow(2.0, iNewBlock - 20));
        };
#endif
