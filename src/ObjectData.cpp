...
void ObjectData::loadFromINIFile(const std::string& filename) {
    ...
    for (const auto& section : objectDataFile.sections()) {
        ...
        if (itemID == ItemID_Invalid) {
            sdl2::log_info("ObjectData::ObjectData(): '{}' is no valid unit/structure name!", sectionName);
            continue;
        }
...
void ObjectData::load(InputStream& stream) {
    for (auto& i : data) {
        for (auto& h : i) {
            ...
            h.prerequisiteStructuresSet = std::bitset<Structure_LastID + 1>(stream.readUint32());
