...
void Deviator::blitToScreen() {
    ...
    const auto dest2   = calcSpriteDrawingRect(
        pTurretGraphic,
        screenborder->world2screenX(realX_ + deviatorTurretOffset[static_cast<int>(drawnAngle_)].x),
        screenborder->world2screenY(realY_ + deviatorTurretOffset[static_cast<int>(drawnAngle_)].y),
        numImagesX_,
        1,
        HAlign::Center,
        VAlign::Center);
