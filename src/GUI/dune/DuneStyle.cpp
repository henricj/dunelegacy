...
unsigned int DuneStyle::getTextHeight(unsigned int FontNum) const {
    if (pFontManager) {
        return pFontManager->getTextHeight(scaled_font_size(FontNum));
    }

    return scaled_font_size(12);
}

unsigned int DuneStyle::getTextWidth(std::string_view text, unsigned int FontNum) const {
    if (pFontManager) {
        return pFontManager->getTextWidth(text, scaled_font_size(FontNum));
    }

    return static_cast<int>(text.length() * scaled_font_size(10));
}
...
void DuneStyle::RenderButton(SDL_Renderer* renderer, const SDL_FRect& dest, SDL_Texture* content, bool pressed) const {
    ...
    if (pressed) {
        ...
        int w, h;
        if (0 == SDL_QueryTexture(content, nullptr, nullptr, &w, &h)) {
            const auto one = static_cast<int>(std::round(scale));

            const auto pressed_src = SDL_Rect{0, 0, w - one, h - one};

            SDL_RenderCopyF(renderer, content, &pressed_src, &pressed_dest);
        } else // ???
            SDL_RenderCopyF(renderer, content, nullptr, &pressed_dest);
    } else
        SDL_RenderCopyF(renderer, content, nullptr, &dest);
}
