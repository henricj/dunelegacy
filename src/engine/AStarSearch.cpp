...
namespace
{
constexpr int MAX_NODES_CHECKED = 128 * 128;
}
...
void AStarSearch::Search(const GameContext& context, UnitBase* pUnit, Coord start, Coord destination) {
#if 1
    ...
    auto* pMap = &context.map;

    auto* const destinationTile = context.map.getTile(destination);

    const FixPoint rotationSpeed =
        1_fix /
        (context.game.objectData.data[pUnit->getItemID()][static_cast<int>(pUnit->getOriginalHouseID())].turnspeed *
         TILESIZE);
