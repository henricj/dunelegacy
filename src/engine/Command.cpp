...
Command::Command(uint8_t playerID, uint8_t* data, uint32_t length) : playerID(playerID) {
    if(length % 4 != 0) { THROW(std::invalid_argument, "Command::Command(): Length must be multiple of 4!"); }

    if(length < 4) { THROW(std::invalid_argument, "Command::Command(): Command must be at least 4 bytes long!"); }

    commandID = static_cast<CMDTYPE>(*reinterpret_cast<uint32_t*>(data));

    if(commandID >= CMDTYPE::CMD_MAX) { THROW(std::invalid_argument, "Command::Command(): CommandID unknown!"); }
...
void Command::executeCommand(const GameContext& context) const {
    ...
    switch(commandID) {
        ...
        case CMDTYPE::CMD_MCV_DEPLOY: {
            ...
            if(pMCV == nullptr) { return; }
            pMCV->doDeploy(context);
        } break;
        ...
        case CMDTYPE::CMD_HARVESTER_RETURN: {
            ...
            if(pHarvester == nullptr) { return; }
            pHarvester->doReturn(context);
        } break;
        ...
        case CMDTYPE::CMD_STRUCTURE_SETDEPLOYPOSITION: {
            ...
            if(pStructure == nullptr) { return; }
            pStructure->doSetDeployPosition(context, static_cast<int>(parameter[1]), static_cast<int>(parameter[2]));
        } break;
        ...
        case CMDTYPE::CMD_BUILDER_PRODUCEITEM: {
            ...
            auto* const pBuilder = objectManager.getObject<BuilderBase>(parameter[0]);
            if(pBuilder == nullptr) { return; }
            pBuilder->doProduceItem(context, static_cast<ItemID_enum>(parameter[1]), static_cast<bool>(parameter[2]));
        } break;
        ...
        case CMDTYPE::CMD_STARPORT_PLACEORDER: {
            ...
            if(pStarport == nullptr) { return; }
            pStarport->doPlaceOrder(context.game);
        } break;
        ...
        case CMDTYPE::CMD_TEST_SYNC: {
            ...
            if(currentSeed[0] != parameter[0]) {
                Dune::Logger.log("Warning: Game is asynchronous in game cycle %d! Saved seed and current seed do not "
                               "match: %ud != %ud",
