...
/**
    ...
    \param getchar  A functor that reads characters from the file.  Returns EOF when done.
    \param bWhitespace
*/
INIFile::INIFile(std::function<int()> getchar, bool bWhitespace)
 : firstLine(nullptr), sectionRoot(nullptr), bWhitespace(bWhitespace) {
    readfile(getchar);
}
...
/**
    ...
    \param  writeString
    \param  bDOSLineEnding
    \return true on success otherwise false.
*/
bool INIFile::saveChangesTo(std::function<int(std::string_view)> writeString, bool bDOSLineEnding) const {
    INIFileLine* curLine = firstLine;

    bool error = false;
    while(curLine != nullptr) {
        unsigned int written = writeString(curLine->completeLine);
        if(written != curLine->completeLine.size()) {
            Dune::Logger.log("INIFile write failed");
            error = true;
        }

        if(bDOSLineEnding) {
            // when dos line ending we also put it at the end of the file
            if((written = writeString("\r\n")) != 2) {
                error = true;
            }
...
void INIFile::readfile(std::function<int()> getchar) {
    sectionRoot = new Section("", INVALID_LINE, 0, 0, bWhitespace);
    ...
    while(!readfinished) {
        ...
        while(true) {
            ...
            tmp = static_cast<unsigned char>(ch);
