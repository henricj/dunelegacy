...
void ObjectData::loadFromINIFile(const INIFile& objectDataFile) {
    ...
    for(INIFile::Section& section : objectDataFile) {
        ...
        if(sectionName.empty() || sectionName == "default structure" || sectionName == "default unit") { continue; }

        auto itemID = getItemIDByName(sectionName);

        if(itemID == ItemID_Invalid) {
            Dune::Logger.log("ObjectData::ObjectData(): '%s' is no valid unit/structure name!", sectionName.c_str());
            continue;
        }
...
int ObjectData::loadItemID(const INIFile& objectDataFile, const std::string& section, const std::string& key,
    ...
    if(strItem.empty()) { return defaultValue; }
    if(strToLower(strItem) == "invalid") { return ItemID_Invalid; }
...
ObjectData::loadPrerequisiteStructuresSet(const INIFile& objectDataFile, const std::string& section,
    ...
    if(strList.empty()) { return defaultValue; }
    if(trim(strToLower(strList)) == "invalid") { return resultSet; }
