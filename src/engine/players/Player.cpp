...
Player::Player(const GameContext& context, House* associatedHouse, std::string_view playername, const Random& random)
    : pHouse(associatedHouse), playerID(0), playername{playername}, random_{random}, context_{context} { }
...
Player::~Player() { context_.game.unregisterPlayer(this); }
...
const GameInitSettings& Player::getGameInitSettings() const { return context_.game.getGameInitSettings(); }
...
const RobustList<const StructureBase*>& Player::getStructureList() {
    return reinterpret_cast<const RobustList<const StructureBase*>&>(context_.game.structureList);
}

const RobustList<const UnitBase*>& Player::getUnitList() const {
    return reinterpret_cast<const RobustList<const UnitBase*>&>(context_.game.unitList);
}

const House* Player::getHouse(HOUSETYPE houseID) const {
    if(static_cast<int>(houseID) < 0 || houseID >= HOUSETYPE::NUM_HOUSES) { return nullptr; }
...
void Player::doSetDeployPosition(const StructureBase* pStructure, int x, int y) const {
    if(pStructure->getOwner() == getHouse() && pStructure->isActive()) {
        const_cast<StructureBase*>(pStructure)->doSetDeployPosition(context_, x, y);
    } else {
...
void Player::doProduceItem(const BuilderBase* pBuilder, ItemID_enum itemID) const {
    if(pBuilder->getOwner() == getHouse() && pBuilder->isActive()) {
        const_cast<BuilderBase*>(pBuilder)->doProduceItem(context_, itemID);
    } else {
...
void Player::doPlaceOrder(const StarPort* pStarport) const {
    if(pStarport->getOwner() == getHouse() && pStarport->isActive()) {
        const_cast<StarPort*>(pStarport)->doPlaceOrder(context_.game);
    } else {
...
void Player::doAttackObject(const TurretBase* pTurret, const ObjectBase* pTargetObject) const {
    if(pTurret->getOwner() == getHouse() && pTurret->isActive()) {
        const_cast<TurretBase*>(pTurret)->doAttackObject(context_, pTargetObject);
    } else {
...
void Player::doReturn(const Harvester* pHarvester) const {
    if(pHarvester->getOwner() == getHouse() && pHarvester->isActive()) {
        const_cast<Harvester*>(pHarvester)->doReturn(context_);
    } else {
...
bool Player::doDeploy(const MCV* pMCV) const {
    if(pMCV->getOwner() == getHouse() && pMCV->isActive()) { return const_cast<MCV*>(pMCV)->doDeploy(context_); }
