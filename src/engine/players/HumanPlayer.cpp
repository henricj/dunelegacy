...
namespace {
constexpr int ATTACKNOTIFICATIONTIME = MILLI2CYCLES(2 * 60 * 1000);
}
...
void HumanPlayer::onProduceItem(ItemID_enum itemID) {
    if(!settings.general.showTutorialHints || (currentGame->gameState != GameState::Running)) { return; }

    if(!isStructure(itemID)) { return; }

    if(itemID == Structure_Slab1 || itemID == Structure_Slab4) { return; }

    if(alreadyShownTutorialHints & (1 << static_cast<int>(TutorialHint::NotEnoughConrete))) { return; }
...
void HumanPlayer::onUnitDeployed(const UnitBase* pUnit) {
    ...
    if(alreadyShownTutorialHints & (1 << static_cast<int>(TutorialHint::HarvestSpice))) { return; }
...
void HumanPlayer::onSelectionChanged(const Dune::selected_set_type& selectedObjectIDs) {
    ...
    for(auto objectID : selectedObjectIDs) {
        ...
        if(pObject->getOwner() != getHouse()) { continue; }
...
void HumanPlayer::triggerStructureTutorialHint(ItemID_enum itemID) {
    if(alreadyShownTutorialHints & (1 << itemID)) { return; }
...
bool HumanPlayer::hasConcreteOfSize(const Coord& concreteSize) const {
    ...
    for(pos.y = 0; pos.y < currentGameMap->getSizeY() - concreteSize.y + 1; pos.y++) {
        for(pos.x = 0; pos.x < currentGameMap->getSizeX() - concreteSize.x + 1; pos.x++) {
            if(hasConcreteAtPositionOfSize(pos, concreteSize)) { return true; }
        }
    }
    return false;
}
