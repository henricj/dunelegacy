...
constexpr char create_customization[] = u8R"(
{
    "version": "1.0",
...
constexpr char generate_customization[] = u8R"(
...
std::array<uint8_t, Random::state_bytes> Random::getState() const {
    ...
    Dune::Logger.log("Getting state %s", to_hex(state));

    return state;
}

void Random::setState(const gsl::span<const uint8_t, state_bytes> state) {
    Dune::Logger.log("Setting state %s", to_hex(state));
...
void RandomFactory::setSeed(gsl::span<const uint8_t> seed) {
    Dune::Logger.log("Setting RandomFactory seed to %s", to_hex(seed));
...
std::vector<uint8_t> RandomFactory::getSeed() const {
    ...
    Dune::Logger.log("Getting RandomFactory seed %s", to_hex(seed));
...
std::vector<uint8_t> RandomFactory::createRandomSeed(std::string_view name) {
    ...
    Dune::Logger.log("Created seed for \"%s\": %s", name, to_hex(output));
...
Random RandomFactory::create(std::string_view name) const {
    ...
    Dune::Logger.log("Created state for \"%s\": %s (from %s)", name, to_hex(buffer), to_hex(seed_));
